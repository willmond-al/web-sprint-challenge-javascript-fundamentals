...
// nestedFuntion can access the variable internal because both are declared inside of myFunction meaning no access is restricted by block/function scope 
...
const zooAnimals = [
  ...
  */
  function animalNames(data){
    ...
    data.forEach(function(item){
      animalName.push(item.animal_name)
    })
    data.forEach(function(item){
      scientificNames.push(item.scientific_name)
    })
  ...
  */
  function USApop(data){
    const totalPop = data.reduce(function(acca, item){
      return acca + item.population
    },0)
    return totalPop
  }
